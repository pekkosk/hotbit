...
class DoubleChiral:
    ...
    def __repr__(self):
        ...
        if self.angle<1E-14:
            ...
            x='DoubleChiral: angle=%.4f (2*pi/%.2f), height=%.4f Ang, x=%.4f' %(self.angle,2*np.pi/self.angle,self.height,self.x)
        return x
    
    
    def _set_table(self):
        if abs(self.x)>1E-12:
            eq = (0,0,int(np.round(2*self.x)))
            self.table = [{'M':2,'equivalent':eq},{'M':1},{'M':np.Inf}]
        else:
            self.table = [{'M':2},{'M':1},{'M':np.Inf}]
    ...
    def transform(self,r,n):
        ...
        R = self.rotation(n)
        return np.dot(R,r) + (0,0,(n[2]+n[0]*self.x)*self.height)
                         
                         
    def rotation(self,n):
        """ Return the (active) rotation matrix for symmetry operation n. """
        angle = n[2]*self.angle + n[0]*(np.pi+self.angle*self.x)
