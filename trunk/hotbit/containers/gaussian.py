...
class Gaussian:
    def __init__(self,atoms,type):
        ...
        self.angle1 = None
        self.angle2 = None
        self.n1 = nu.array([-1,0,0])
        self.n2 = nu.array([0,1,0])
        self.R1 = None
        self.R2 = None
    ...
    def transform(self,r,n):
        ...
        A = nu.array( [0,0,abs(R1)+abs(R2)] ) # another center of curvature
        rot = self.rotation(n)
        if R>0:
            #return nu.array([0,0,0])
            return nu.dot(rot,r)
        else:
            if R>0:
                B=(0,0,R1-R)
    ...
    def rotation(self,n):
        ...
        if n[0]==n[1]==0:
            ...
            axis = nu.array( [R2*b,R1*a,0] )
            angle = ( a**2*R1+b**2*abs(R2) )/nu.sqrt( (a*R1)**2+(b*R2)**2 )
