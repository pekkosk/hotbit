...
class RepulsiveFitting:
    ...
    def add_point(self,data,comment=None):
    ...
    def use_energy_curve(traj,edft,charge,bonds,weight):
        ...
        for i,atoms in enumerate(trajectory):
            ...
            rl.append(nu.average(vec(lengths)))
...
if __name__=='__main__':
    ...
    rep=RepulsiveFitting(['C','Au'],r_cut=2.6459,r_dimer=1.85899,calc=calc)
    rep.use_dimer(1.0)
    rep.add_point([2.2,-8,0.8,'added'])
    #rep.use_energy_curve('traj.xyz',edft,charge=0,bonds=[(0,1)])
    #rep.use_equilibrium_atoms('cluster.xyz')
    rep.fit()
