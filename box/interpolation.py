...
#        raise AssertionError('Error in binding bin for %f' %x)
    ...
    def __call__(self,x,der=None):
        ...
        if isinstance(x, np.ndarray):
            hi = np.where(hi == len(self.x), hi-1, hi)
        else:
            if hi == len(self.x):
                hi -= 1
...
class BilinearInterpolation:
    ...
    def __init__(self,a,xgrid=None,ygrid=None):
        raise AssertionError('Use the same function from helper-module, not box; this function is somehow broken.')
        self.a=a
        self.nx=a.shape[0]
        self.ny=a.shape[1]
        self.xgrid=xgrid
    ...
    def __call__(self,x,y):
        ...
        i,j=(np.floor(x/self.dx),np.floor(y/self.dx))
...
def interpolate_path(points, N):
    ...
    label_points = np.zeros(len(points)) # the distances where the fixed points are on the path
    for i in range(1, len(label_points)):
        label_points[i] = np.sum(lengths[:i])
    dk = np.sum(lengths)/(N-1.0) # maximum distance between points
    ...
    for i in range(len(points)-1):
        A = points[i]
        B = points[i+1]
        X = np.linspace(A[0], B[0], N_i[i])
        Y = np.linspace(A[1], B[1], N_i[i])
        Z = np.linspace(A[2], B[2], N_i[i])
        for x,y,z in zip(X[1:], Y[1:], Z[1:]):
            path.append((x,y,z))
    distances = np.zeros(len(path))
    for i in range(len(path[1:])):
        distances[i+1] = distances[i] + norm(np.array(path[i+1])-np.array(path[i]))
    return np.array(path), distances, label_points
...
if __name__=='__main__':
    ...
    if False:
        ...
        #for t in np.linspace(0,1,10):
            ...
            #print t, f.line_integral(b=t) #,np.sqrt(5.0)
