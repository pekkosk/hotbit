...
calc = Hotbit(SCC=False,txt='tmp.cal',kpts=(6,1,1))
...
#view(atoms)
calc = Hotbit(SCC=False,txt='tmp.cal')
atoms6.set_calculator(calc)
e6 = atoms6.get_potential_energy()

assert abs(6*e1-e6)<1E-12
...
atoms.rattle(0.1)
assert check_energy_conservation(atoms,dt=0.2*fs,steps=30,tol=0.01)
