...
class LEPS_Calculator:
    ...
    def _V_LEPS(self,rab,rbc):
        ...
        """
        def Q(r,alpha,r0,d): return d/2.*( 3./2*np.exp(-2*alpha*(r-r0))-np.exp(-alpha*(r-r0)) )
        def J(r,alpha,r0,d): return d/4.*( np.exp(-2*alpha*(r-r0))-6*np.exp(-alpha*(r-r0)) )
        ...
        return Qab*ai+Qbc*bi+Qac*ci-np.sqrt(Jab**2*ai**2+Jbc**2*bi**2+Jac**2*ci**2\
...
class MEP:
    ...
    def set_positions(self,positions):
        ...
        if self.lock: 
            self.positions2=np.zeros((self.M,self.N,3))
            n1 = 0
    ...
    def get_constraint_deviations(self):
        ...
        try:
            ...
            R.shape=(self.M,self.N*3)      
        for i in range(self.M-1):
            Di=np.linalg.norm(R[i+1]-R[i])
            L+=Di
            dev.append( Di )
        return np.array(dev) - L/(self.M-1)           
